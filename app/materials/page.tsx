...
export default function MaterialsPage() {
  ...
  const filteredLends = lends.filter((lend) => lend.item.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const handleStockAdjustment = (materialId: number, action: "add" | "remove") => {
    ...
    setConfirmDialog({
      open: true,
      title: `${action === "add" ? "Adicionar" : "Remover"} Item`,
      description: `Deseja ${action === "add" ? "adicionar" : "remover"} uma unidade de ${material.name}?`,
      onConfirm: () => {
        setMaterials(
          materials.map((m) =>
            m.id === materialId
              ? { ...m, quantity: action === "add" ? m.quantity + 1 : Math.max(0, m.quantity - 1) }
              : m,
          ),
        )
                  ...
                  >
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-3 py-1 bg-gray-100 rounded text-sm font-medium">{material.quantity}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          className="flex-1 bg-transparent"
                ...
                >
                  <div className="grid gap-4">
                    <div className="grid gap-2">
                      ...
                      <Input name="edit-borrower" defaultValue={selectedLend.borrower} />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="edit-due-date">Data de Entrega</Label>
                      <Input name="edit-due-date" type="date" defaultValue={selectedLend.dueDate} />
                    </div>
                    <div className="grid gap-2">
